...
describe("RecipePreparationStepRepository", () => {
  ...
  describe("saveAllForRecipe()", () => {
    test("Ruft batchUpsert auf und gibt neu geladene Daten zurück", async () => {
      ...
      // 1. getStepsForRecipe (Tabelle) — Bestehende laden
      // 2. batchUpsert (Tabelle) — upsert().select()
      // 3. getStepsForRecipe (Tabelle) — Reload
      ...
      supabaseMock.client.from
        .mockReturnValueOnce(loadMock1)   // 1. getStepsForRecipe
        .mockReturnValueOnce(upsertMock)  // 2. batchUpsert
        .mockReturnValueOnce(loadMock2);  // 3. getStepsForRecipe (Reload)
    ...
    test("Löscht entfernte Schritte per batchRemove und upserted neue", async () => {
      ...
      supabaseMock.client.from
        ...
        .mockReturnValueOnce(loadMock2);  // 4. getStepsForRecipe (Reload)

      const result = await repo.saveAllForRecipe("recipe-uuid-001", [testDomain], authUser);

      // batchRemove: delete().in() für den entfernten Schritt
