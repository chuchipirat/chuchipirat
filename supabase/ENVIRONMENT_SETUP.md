...
| `send-welcome-email`        | Willkommens-E-Mail bei neuer Registrierung. Wird automatisch via Postgres-Trigger (pg_net) bei User-INSERT ausgelöst                                                                                                            | `SUPABASE_SERVICE_ROLE_KEY`, `SITE_URL`, `BREVO_API_KEY` oder `SMTP_HOST`/`SMTP_PORT`                        |
...
| `cron-event-review-email`   | Tägliche Feedback-E-Mail an Köche von gestern beendeten Events (01:00 UTC)                                                                                                                                                      | `SUPABASE_SERVICE_ROLE_KEY`, `SITE_URL`, `SENTRY_DSN`, `BREVO_API_KEY` oder `SMTP_HOST`/`SMTP_PORT`          |
...
- [x] `SITE_URL` in `.env` setzen (z.B. `https://chuchipirat.ch`) — bereits als Pflicht-Variable für Auth (GoTrue) vorhanden, wird von Edge Functions für E-Mail-Links/CORS-Origin mitverwendet (kein separates `APP_URL` mehr)
