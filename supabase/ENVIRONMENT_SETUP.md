...
| `20260330000001_product_qa_columns.sql`                      | QA-Spalten auf `products`: `qa_checked` (BOOLEAN) + `qa_checked_at` (TIMESTAMPTZ) für Qualitätssicherungs-Tracking. Index auf `qa_checked`                                                                                                                                                                                                                           |
| `20260330000002_enable_pg_trgm.sql`                          | pg_trgm Extension + GIN-Trigram-Index auf `products.name` für Fuzzy-Matching bei Duplikaterkennung                                                                                                                                                                                                                                                                    |
| `20260330000003_create_product_synonyms.sql`                 | Tabelle `product_synonyms` für manuelle Synonym-Paare (z.B. Rüebli/Karotten). UNIQUE-Constraint, RLS (SELECT alle, INSERT/UPDATE/DELETE nur Admins), Audit-Trigger                                                                                                                                                                                                   |
| `20260330000004_find_similar_products_rpc.sql`               | SECURITY DEFINER Funktion `find_similar_products(similarity_threshold)`: kombiniert pg_trgm-Ähnlichkeit (Self-Join) und Synonym-Paare, dedupliziert, gibt JSONB-Array mit Produkt-Paaren, Ähnlichkeitswert und Match-Typ zurück. Standard-Schwellenwert 0.3                                                                                                           |
| `20260330000005_create_product_duplicate_dismissals.sql`     | Tabelle `product_duplicate_dismissals` für bestätigte Nicht-Duplikate. Normalisierte Paar-IDs (CHECK product_a_id < product_b_id), UNIQUE-Constraint, CASCADE bei Produkt-Löschung. RLS: SELECT alle, INSERT/DELETE nur Admins                                                                                                                                       |
| `20260330000006_find_similar_products_exclude_dismissed.sql`  | Aktualisiert `find_similar_products()`: LEFT JOIN gegen `product_duplicate_dismissals` — bestätigte Paare werden automatisch aus den Ergebnissen ausgeschlossen                                                                                                                                                                                                        |
