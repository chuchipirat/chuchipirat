...
- **Anlass anlegen ist nicht atomar → Anlässe ohne Zeitscheibe** (`src/components/Event/Event/createNewEvent.tsx`, `saveEvent()` ~l.353–395; `EventRepository.saveDates()` ~l.532–556) — Event, Koch und Zeitscheiben werden in getrennten Requests angelegt, und `saveDates` löscht erst alle Zeitscheiben und fügt dann neu ein. Ein Abbruch dazwischen (Verbindung, geschlossener Tab, RLS-Fehler) hinterlässt einen Anlass ohne Zeitscheibe bzw. ohne Koch. Die Datenintegritätsseite kann diese Anlässe seit der Migration `20260919000001` finden und löschen, die Ursache bleibt. Lösung: `create_event`-RPC (Event + Koch + Zeitscheiben + Menüplan-Init in einer Transaktion) und Zeitscheiben per RPC atomar ersetzen. Aufwand inkl. Migration und Anpassung von `createNewEvent.tsx`/`event.tsx`.
  **Priorität:** mittel · **Komplexität:** mittel
