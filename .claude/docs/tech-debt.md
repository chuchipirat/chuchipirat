...
- **`event.tsx` — Hauptthread-Blockierung beim initialen Laden (CHUCHIPIRAT-AD)** — Sentry meldet "Degraded UI Performance": Hauptthread beim Navigieren zu `/event/*` bis zu 1.27s blockiert (7× seit 2026-06-09, 0 betroffene Nutzer, AI-detected, keine Regression). Ursache: ~14 unabhängige `useEffect`-Hooks in `event.tsx` laden Event-Daten separat und feuern beim initialen Laden ~13 parallele Requests; jeder `dispatch()` in den gemeinsamen Reducer re-rendert den kompletten 2'557-Zeilen-Baum inkl. des aktiven Tab-Panels (`MenuplanPage` als Default), da weder die Tab-Seiten (`MenuplanPage`, `EventShoppingListPage`, `EventMaterialListPage`, `EventUsedRecipesPage`, `EventGroupConfigurationPage`) noch die an sie durchgereichten Callback-Props (`onMenuplanUpdate`, `onRecipeUpdate`, `fetchMissingData`, `onMasterdataCreate` u.a.) memoisiert sind (`React.memo`/`useCallback`) — ~13 volle Re-Renders in <2s ergeben plausibel die gemeldeten Long-Animation-Frames. Fix: aktives Tab-Panel mit `React.memo` umschliessen und die an es durchgereichten Funktions-Props mit `useCallback` stabilisieren (analog zu bereits memoisierten Komponenten wie `MealPlanPanel`, `HomeOngoingEventSection`). Bewusst nicht sofort angegangen (2026-09-18) — Risiko/Aufwand in dieser zentralen, business-kritischen Datei steht in keinem Verhältnis zum Impact (0 betroffene Nutzer). Hängt mit dem bestehenden Split-Eintrag für `event.tsx` weiter unten zusammen — eine Aufteilung in Tab-Komponenten würde die Memoisierung natürlich mit sich bringen.
  **Priorität:** tief · **Komplexität:** mittel (Scoped: nur `MenuplanPage` + 4 Callbacks) bis gross (alle Tab-Panels)
...
- **Ausgaben-Liste bei > 1000 Zeilen abgeschnitten** — `ExpenseRepository.getExpensesForEvent()` und `BudgetRepository.getBudgetsForEvent()` nutzen `findMany()` ohne eigene Pagination (siehe Eintrag oben). Hat ein Anlass mehr als 1000 Budgets oder Ausgaben, werden die überzähligen Zeilen stillschweigend nicht geladen und fehlen entsprechend in Summen und Fortschrittsanzeige. In der Praxis unkritisch (ein Lager hat Hunderte, nicht Tausende Ausgaben).
...
- **DAL wirft rohe Nicht-`Error`-Objekte** — `BaseRepository.findById()` / `findMany()` / `create()` etc. reichen das Supabase-`error`-Objekt (`{code, details, hint, message}`) via `if (error) throw error` unverändert weiter. Aufrufer, die es an `Sentry.captureException()` geben, erzeugen unbrauchbare Gruppen ("Object captured as exception with keys ..."). Aktuell wird das an einer Stelle (`globalSettingsContext.tsx`) über `toError()` aus `src/utils/errorUtils.ts` normalisiert. Langfristig sollte die Normalisierung zentral in `BaseRepository` passieren (einmal `throw toError(error)` statt an jeder der ~263 `captureException`-Aufrufstellen). Gleiches gilt für `isTransientNetworkError()`: vorübergehende `Failed to fetch`-Fehler aus Hintergrund-Polls (`Home.tsx:806/1033`, weitere) sollten dort ebenfalls nicht nach Sentry.
  `DonationRepository` wurde am 2026-09-18 gefixt (CHUCHIPIRAT-FY: alle 9 `catch`-Blöcke filtern jetzt via `isTransientNetworkError()`/`isMissingSessionError()`, bevor sie an Sentry melden — Rohobjekt wird weiterhin ungefiltert geworfen). Noch offen mit demselben unbedingten `Sentry.captureException(error)`-Muster: `RequestRepository`, `MenuplanRepository`, `MaterialListRepository`, `RecipeCommentRepository`, `RequestCommentRepository`.
  **Priorität:** tief · **Komplexität:** mittel

- **Kein flächendeckender `isUuid()`-Guard vor `.eq("*_uid"/"user_id", …)`** — Aus veralteten localStorage-Caches oder Links kann eine Firebase-UID als ID durchsickern und löst dann Postgres `22P02` ("invalid input syntax for type uuid") aus. Root Cause für `authUser.uid` ist per `isValidCachedAuthUser` (`authUserContext.tsx`) gefixt; als Defense-in-depth haben `EventRepository.getAllEventsForUser`, `EventRepository.getEvent` (2026-09-18, CHUCHIPIRAT-BF — veralteter `/event/<firebase-uid>`-Link führte zu einem 406 von PostgREST statt der erwarteten "nicht gefunden"-Meldung) und `DonationRepository.getMyDonations` einen `isUuid()`-Guard (`src/utils/uuid.ts`). Noch offen: ein Audit aller weiteren Repository-Methoden, die eine User-/Objekt-UID direkt in `.eq()` / `.in()` gegen eine `uuid`-Spalte reichen (z.B. `RecipeRepository.searchByCreatorId(s)`, diverse `*_uid`-Filter), und ggf. dort denselben Guard ergänzen.
  **Priorität:** tief · **Komplexität:** klein

- **`catch { Sentry.captureException(error); throw error; }`-Doppelmeldung in Repositories** — Etliche Repository-Methoden fangen den Fehler nur, um ihn zu melden, und werfen ihn dann weiter — der Aufrufer meldet ihn ein zweites (bei `insertFeed` → `getFeedById` sogar ein drittes) Mal. `FeedRepository` wurde bereinigt (meldet nicht mehr selbst; Aktivitäts-Feeds laufen über `postActivityFeed`, `src/components/Shared/feedActivity.ts`). `DonationRepository.getEventDonations()` wurde bereinigt (2026-09-05, im Zuge von Issue #86 / Epic 0.1 — meldet nicht mehr selbst; beide Aufrufer `eventInfo.tsx` und `expenseTracking.tsx` melden in der aufrufenden Schicht). Gleiches Muster noch in den übrigen `DonationRepository`-Methoden (`getMyDonations`, `getAllDonations` u.a.) sowie in `MaterialListRepository`, `MenuplanRepository`, `RequestRepository`, `RecipeRepository` u.a. Sweep: Die Repository-Schicht meldet nicht selbst an Sentry — sie normalisiert höchstens via `toError()` und wirft; die aufrufende Schicht entscheidet über die Meldung.
  **Priorität:** tief · **Komplexität:** mittel
...
- **`recipe.edit.tsx`-Reducer mutiert State in-place** — Der `recipeEditReducer` (und `onPostionMoreContextMenuClick`) kopieren `state.recipe.ingredients` / `preparationSteps` / `materials` nur flach (`{...state.recipe.ingredients}`) und mutieren dann `.entries[uid][field] = …` bzw. `.order.push()/.splice()` direkt auf den State-Referenzen (z.B. Z. ~355–381, ~433–460, ~1496–1546, Z. 392 kopiert gar nicht). Dadurch können `order` und `entries` bei rasch aufeinanderfolgenden oder unterbrochenen Aktionen desynchronisieren — ein `order`-Eintrag ohne passenden `entries`-Eintrag (bzw. ein `entries`-Eintrag mit nur `uid` aus `regenerateEntryUids`) hat CHUCHIPIRAT-G3 (Render, `ingredient.uid` auf `undefined`), CHUCHIPIRAT-H0 (`ON_INGREDIENT_CHANGE`-Reduce über `order` mit `entries[uid].posType`), CHUCHIPIRAT-H5 (`Recipe.deleteEmptyIngredients` / `checkRecipeData` beim Speichern) und CHUCHIPIRAT-H6 (`ProductAutocomplete`-Render, `product.name` auf `undefined`) ausgelöst. Übergangsweise: Render-Guard (`if (!ingredient) return null`), `LastCardMoved`-Guard, `entries[uid]?.posType`-Optional-Chaining in den `*_CHANGE`-Reduces, **`Recipe.repairPositionStructure()`** (normalisiert `order`/`entries`: verwaiste order-Einträge raus, order-Duplikate raus, entries-ohne-order raus, `product`/`material` auffüllen) aufgerufen in `fromRepositoryData`, `prepareSave` sowie im Reducer bei `ON_UPDATE_LIST`/`DRAG_AND_DROP_UDPATE`, plus `!entry`-Guards in `deleteEmpty*` / `to*Rows` / `regenerateEntryUids`. Sauberer Fix bleibt: Reducer vollständig immutabel machen (`order` als `[...]`, `entries` als `{...}` kopieren, Positions-Objekte vor Mutation klonen) — grösserer, eigener PR, alle 3 Blöcke betroffen.
  **Priorität:** mittel · **Komplexität:** gross

- **Menuplan-Editor mutiert State in-place** — Gleiche Klasse wie der `recipe.edit`-Reducer. `menuplanService.ts`-Funktionen (`addMealType` war betroffen, gefixt; `deleteMealType`, `onMealTypeUpdate`/`tempMealTypes` in `useMenuplanHandlers.tsx` u.a.) kopieren `{...mealTypes}` flach und mutieren dann `.order`/`.entries`. `order.push()` ohne Guard konnte eine `uid` doppelt in `mealTypes.order` legen → beim Speichern `duplicate key … event_meal_types_pkey` (23505, CHUCHIPIRAT-GE). Übergangsweise: `addMealType` immutabel + `!order.includes`-Guard, und `MenuplanRepository.saveMenuplan` dedupliziert alle Collections via `dedupeByUid` (heilt auch bereits verdoppelten Client-State). Es existiert bereits eine Render-Zeit-Telemetrie (`menuplan.tsx` Z. ~219: `captureMessage("Doppelte MealTypes im Menüplan")`). Sauberer Fix: `useMenuplanHandlers` + `menuplanService` durchgängig immutabel — bleibt offen (verbessert React-Renderverhalten/Korrektheit generell, ist aber für die 23505-Fehlerklasse nicht mehr die kritische Ursache, siehe folgender Eintrag).
  **Priorität:** tief · **Komplexität:** gross

- **~~`save_menuplan()` hatte keine Sperre gegen konkurrierende Aufrufe~~ — GELÖST (CHUCHIPIRAT-H8, 2026-09-18)** — Trotz des `dedupeByUid`-Fixes oben (Aug 28) trat `duplicate key … event_meal_types_pkey` am 9. Sept. erneut auf. Root Cause war nicht (nur) In-Place-Mutation: `dedupeByUid` schützt nur vor Duplikaten INNERHALB eines Aufrufs, nicht vor zwei echten, unabhängigen `save_menuplan`-Aufrufen für dasselbe Event (mehrere Köch:innen/Tabs, schnelle Doppel-Saves) — unter READ COMMITTED sehen sich zwei gleichzeitige Transaktionen nicht, beide löschen "alles", beide fügen dieselbe (stabile, clientseitig wiederverwendete) mealType-UID neu ein, wer zuletzt committet verletzt den Unique-Key. `save_shopping_list_items()` hatte exakt dasselbe Muster bereits am 6. Sept. mit einem transaktionsgebundenen Advisory Lock (`pg_advisory_xact_lock(hashtext(p_list_id))`) gelöst — dessen Migrationskommentar nennt `save_menuplan` sogar explizit als Vorbild, wurde aber nie nachgezogen. Gefixt in `20260918000001_save_menuplan_advisory_lock.sql` (gleiches Muster, Lock-Key = `event_id`). Lokal verifiziert: zwei Sessions mit `pg_advisory_xact_lock` auf dieselbe `event_id` serialisieren korrekt (zweite Session blockiert bis COMMIT der ersten).

- **~~Einkaufsliste/Materialliste: Item-Edits über whole-list delete+re-insert~~ — GELÖST (PR #227)** — Track B (`fix/shopping-list-save-race`, RPC mit `pg_advisory_xact_lock`, Save-Zähler, Realtime-Skip, Kontextmenü-Delete-Persist, Subscription-Leak) hat die reproduzierten Symptome entschärft. Track A (`refactor/shopping-list-surgical-writes`, PR #227) hat das Grundmuster ersetzt: **stabile client-generierte Row-IDs** (`crypto.randomUUID()`, Muster `save_menuplan`) ab Erstellung + `ShoppingList/MaterialList.carryOverItemIds` bei der Neuberechnung; **nebenläufigkeitssichere Diff-RPCs** `save_shopping_list_items` / `save_material_list_items(p_list_id, p_items, p_known_ids)` — löschen serverseitig nur IDs, die der Client kannte und weglässt (Fremd-Adds überleben), `ON CONFLICT DO UPDATE … WHERE row-is-distinct` (unveränderte Zeilen = kein Write/Echo); `knownIds` aus DB-Snapshot-Refs (`shoppingListRef` / neuer `materialListRef`) via `getPersistedItemIds`-Prop; `shoppingListItemKey`/Render-Keys/Highlight auf `item.id`; Materiallisten-Positionen jetzt für alle angezeigten Listen live (`subscribeToItemsForLists`); `moveItemToDepartment`-unit-Bug mitgefixt. Statt per-Handler-Chirurgie bewusst „Client schickt weiterhin die ganze Liste, Server difft" — hält das Regressionsrisiko am 1800-Zeilen-Handler klein. Toten `ShoppingListRepository.updateItem` entfernt (Checkbox-Schnellspur `updateItemChecked` bleibt).
...
- **Anlass atomar anlegen und Zeitscheiben atomar ersetzen (Ursache der Anlässe ohne Zeitscheibe)** — Die Datenintegritätsseite findet und löscht solche Anlässe seit der Migration `20260919000001`, die Ursache bleibt und erzeugt weiter Waisen.
  ...
  **Lösung:**
  1. RPC `create_event(...)`: Event, Ersteller und weitere Köch:innen, Zeitscheiben, Gruppenkonfiguration und Menüplan-Initialisierung in **einer** Transaktion. Vorbild ist `copy_event(TEXT, JSONB, JSONB)` (`20260415000001_copy_event_function.sql`) für Aufbau, Sicherheitsmodell und Fehlermeldungen. Prüfen, ob sich `initializeMenuplan` sauber hineinziehen lässt (Baustein `save_menuplan`).
  2. RPC `save_event_dates(event_id, dates jsonb)`: Zeitscheiben atomar ersetzen, mindestens eine Zeitscheibe Pflicht (`RAISE EXCEPTION`), `date_from <= date_to`.
  3. Bild-Upload bleibt ausserhalb der Transaktion (Storage) und folgt nach `create_event`. Scheitert er, existiert ein vollständiger Anlass ohne Cover.
  4. Frontend: `saveEvent()` auf einen RPC-Aufruf reduzieren (Feed und Analytics bleiben), `event.tsx` nutzt `save_event_dates`.
  5. Optional, sobald PROD bereinigt ist: Sicherheitsnetz in der DB (verzögerter Constraint-Trigger «jeder Anlass hat mindestens eine Zeitscheibe»).
  6. Tests: SQL-Transaktionstest (ungültiger Bereich → nichts wird angelegt), Repository-Test, Wizard-Test.
  7. Nach dem Deploy: Datenintegrität auf PROD ausführen und verbliebene Waisen löschen.
  **Priorität:** mittel · **Komplexität:** gross

- **Produkte/Materialien lassen sich löschen, obwohl Rezepte sie noch verwenden** (`src/components/Product/products.tsx` ~l.225–262, `src/components/Material/materials.tsx` ~l.505–535; FKs `recipe_ingredients.product_id` und `recipe_materials.material_id` sind `ON DELETE SET NULL`) — Die Where-Used-Warnung verhindert das Löschen nicht, nur die Verwendung im Menüplan blockiert (`RESTRICT`). Die Rezeptzeile bleibt mit Menge, Einheit und Detail-Text bestehen, aber ohne Produkt, und der **Name geht verloren**. Solche Rezepte zeigen eine Zutat ohne Namen. Nach Code-Lektüre (nicht ausgeführt) wirft die Einkaufslisten-Erzeugung dabei einen `TypeError`: `shoppingList.class.ts` ~l.300 findet das Produkt per ID nicht, `addItem` kehrt bei fehlendem Item still zurück, aber `addTraceEntry` (~l.559) liest danach `item.uid` von `undefined`. Einkaufs- und Materiallisten sind nicht betroffen: Ihr `CHECK` (`chk_item_source`, `chk_material_item_source`) lässt kein `NULL`-Produkt zu, das Löschen schlägt dort fehl.
  **Optionen:** (a) Löschen im UI sperren, solange Rezepte das Element verwenden, und auf «Items zusammenführen» verweisen (`merge_products`/`merge_materials` setzen die Referenzen um); (b) die beiden FKs auf `RESTRICT` ändern (Schemaänderung auf PROD, betrifft Merge- und Cleanup-Funktionen); (c) beim Löschen den Namen mitspeichern (Vorbild `event_menue_recipes.deleted_recipe_name`), damit die Zeile für den Admin lesbar bleibt. Erkennen lassen sich die Fälle mit den Prüfungen «Rezept-Zutaten ohne Produkt» und «Rezept-Materialien ohne Material» auf der Datenintegritätsseite (Migration `20260919000002`).
  **Zweite Quelle für Zutaten ohne Produkt:** Der Rezept-Editor speichert eine Zutat, sobald Menge oder Einheit gesetzt sind, auch ohne Produkt (`Recipe.deleteEmptyIngredients` in `recipe.class.ts` ~l.842 entfernt nur Zeilen ohne Menge, Einheit *und* Produkt). Eine Validierung «Produkt ist Pflicht» beim Speichern würde diese Quelle schliessen, unabhängig von den Optionen oben. Bei Materialien gibt es sie nicht (`deleteEmptyMaterials` entfernt Zeilen ohne Material).
  **Priorität:** mittel · **Komplexität:** mittel

- **Löschen eines Anlasses hinterlässt Storage- und Feed-Waisen** — Die Cleanups `cleanup_events_without_dates/-cooks` (und das normale Löschen über die Event-Seite) entfernen das Cover-Bild im Storage nicht, wenn die löschende Person nicht Koch des Anlasses ist: `media_events_delete` (`20260401000007_grants_indexes.sql` ~l.393) erlaubt nur `is_event_cook(folder)`, kein Admin. Ausserdem verweist `feeds` nur per Text-ID (`source_object_uid`) auf Anlässe, Feed-Einträge gelöschter Anlässe bleiben stehen. Lösung: `OR is_admin()` in der Storage-Policy und ein Housekeeping-Schritt für verwaiste Feed-Einträge (siehe `cron-housekeeping`).
  **Priorität:** tief · **Komplexität:** klein

- **`check_duplicate_emails()` liefert `NULL` statt `[]`, wenn es keine Duplikate gibt** (`supabase/migrations/20260401000003_functions.sql`, ~l.1174) — Das `COALESCE(jsonb_agg(...), '[]')` steht in einer Unterabfrage mit `GROUP BY`, die bei null Zeilen gar keine Zeile liefert. Die Seite fängt das mit `data ?? []` ab, andere Aufrufer würden `NULL` erhalten. Lösung: Aggregation ohne `GROUP BY` oder äusseres `COALESCE` um die Unterabfrage.
  **Priorität:** tief · **Komplexität:** klein

- **`materialList.tsx` — Realtime-Effect subscribt bei jedem Render neu** — `useRealtimeConnectionStatus()` (`Shared/useRealtimeConnectionStatus.ts`) gibt bei jedem Render ein **neues Objekt** zurück (`return {overallStatus, register, unregister, setStatus, retryAll}`), nur die enthaltenen Funktionen sind per `useCallback` stabil (per Test verifiziert). Der Item-Subscription-Effect in `materialList.tsx` (~Z. 421) führt `realtime` im Dependency-Array, wodurch Channel und Bindings bei jedem Re-Render ab- und neu aufgebaut werden. Fix: `const {setStatus, register, unregister} = realtime;` und nur diese Funktionen als Dependencies verwenden (oder den Hook-Rückgabewert mit `useMemo` stabilisieren — behebt alle Aufrufer auf einmal).
  **Priorität:** mittel · **Komplexität:** klein
...
- **Mail-Konsole schreibt pro Versand zwei `mail_log`-Zeilen** (`src/components/Admin/mailConsole.tsx`, `sendMail()`: `database.mailLog.create` im Erfolgs- und im Fehlerpfad, `templateName: "newsletter"`) — Die Edge Function `send-mail` protokolliert jeden Versand bereits selbst (`template_name: "admin_console"`, inkl. `details` mit Transport und Footer-Einstellung). Die Mailbox-Übersicht zeigt jeden Versand deshalb doppelt, einmal mit zusätzlichen Details, einmal ohne. Client-seitiges Logging entfernen (Fehlerfall: die Edge Function loggt Fehler von `sendBulkEmail` selbst, nur Fehler *vor* dem Versand wie 400/403 hinterlassen heute nur den Client-Eintrag — vor dem Entfernen klären, ob diese sichtbar bleiben sollen).
  **Priorität:** mittel · **Komplexität:** klein
