...
- **Einkaufsliste/Materialliste: Item-Edits über whole-list delete+re-insert statt surgical writes** — Jede Positions-Mutation (Menge, Einheit, Abteilung, Löschen, Duplikat-Merge) in `useShoppingListHandlers.tsx` / `useMaterialListHandlers.tsx` läuft über `persistListItems` → `saveListItems` (ersetzt alle Zeilen der Liste, neue UUIDs). Die Realtime-Subscription ersetzt den lokalen State bei jedem Echo. Ein Hotfix (Branch `fix/shopping-list-save-race`, RPC `save_shopping_list_items` mit `pg_advisory_xact_lock` + Transaktion, Save-Flag als synchroner Zähler, Realtime-Skip während Save, Kontextmenü-Delete persistiert jetzt, Subscription-Leak geschlossen) macht die reproduzierten Symptome (Duplikat-Zeilen, zurückspringende Edits) unmöglich. Sauberer Fix (**Track A**, eigener `refactor/`-Branch gegen `develop`): stabile client-generierte Row-IDs (wie `save_menuplan`), `ShoppingListRepository.updateItem` (existiert, ungenutzt) + neue `deleteItem`/`insertItem` verdrahten, `saveListItems`/RPC nur noch für volle Regeneration, alle Mutations-Zweige auf surgical ops, `shoppingListItemKey`/`getChangedShoppingListItemKeys` (`event.tsx`) auf ID-basiert, Realtime-Handler vereinfachen (Echo dann harmlos), Materialliste analog. `moveItemToDepartment` (`useShoppingListHandlers.tsx` ~Z. 281, ignoriert aktuell `unit`) mitfixen. Toter `state.shoppingList.unsubscribe`-State (durch Ref ersetzt) mit entfernen. Plan: `~/.claude/plans/linked-tinkering-pelican.md`.
  **Priorität:** hoch · **Komplexität:** gross
